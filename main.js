...
const hiddenDateInput = document.getElementById('hidden-booking-date');
const hiddenTimeInput = document.getElementById('hidden-booking-time');
...
// Updated function to render slots with availability logic
async function renderTimeSlots() {
    ...
    standardTimeSlots.forEach(slot => {
        ...
        // Check if this specific date/time is already in the file
        const isBooked = bookings.some(b => 
            b.includes(`Date: ${selectedFormattedDate}`) && 
            b.includes(`Time: ${slot}`)
        );
        
        if (isBooked) {
            btn.disabled = true; // Makes it unclickable
            btn.classList.add('booked'); // CSS class for red/disabled look
...
form.addEventListener('submit', async (e) => {
    ...
    // 1. DYNAMICALLY find the inputs inside the form at the moment of submission
    ...
    if (!dInput || !dInput.value || !tInput || !tInput.value) {
        alert("Error: Date or Time is missing. Please ensure you have selected a slot.");
        return;
    }
...
btnGoStep2.addEventListener('click', () => { panelStep1.classList.remove('active'); panelStep2.classList.add('active'); });
btnBackStep1.addEventListener('click', () => { panelStep2.classList.remove('active'); panelStep1.classList.add('active'); });
calPrevBtn.addEventListener('click', () => { currentCalendarDate.setMonth(currentCalendarDate.getMonth() - 1); renderCalendar(currentCalendarDate); });
calNextBtn.addEventListener('click', () => { currentCalendarDate.setMonth(currentCalendarDate.getMonth() + 1); renderCalendar(currentCalendarDate); });
